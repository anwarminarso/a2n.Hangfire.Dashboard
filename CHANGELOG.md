...
## 2.6.0-beta.1 — Integrations (pre-release)

> **Pre-release.** First cut of the v2.6 Integrations release. Plugs the dashboard into the modern observability and automation stack with four **independently shippable, opt-in** integrations — OpenTelemetry trace linking, a Prometheus `/metrics` endpoint, a read-only REST API, and CSV/JSON export. Every integration reuses the existing `IStorageQueryProvider` / optional `IStorageMetricsProvider` / core `IMonitoringApi` (no new storage-specific queries), honors the configured dashboard path prefix, targets net8.0/net9.0/net10.0, and enforces an authorization check before returning any Hangfire data. See [docs/integrations-v2-6.md](docs/integrations-v2-6.md) for each endpoint's default authorization mode and how to change it.

### Added

- **OpenTelemetry trace linking** — a new NuGet package `a2n.Hangfire.Dashboard.OpenTelemetry` (not referenced by the core package) that captures the ambient W3C `traceparent` when a job is enqueued (via an `IClientFilter`) and restores it as a child execution span when the job runs (via an `IServerFilter` on a named `ActivitySource`, so hosts can register it with their existing tracer provider). Opt-in via `GlobalConfiguration.Configuration.UseHangfireDashboardOpenTelemetry(...)`. A new core `DashboardUIOptions.TraceLinkBuilder` delegate — with ready-made `TraceLinkBuilders.Tempo` / `Jaeger` / `Honeycomb` / `Template` presets — renders a "View distributed trace →" deep link on the Job Details page (only when a builder is configured *and* a parseable traceparent is present; opens in a new tab). When the package is not registered, jobs enqueue/execute with no stored traceparent and all other features are unchanged.
- **Prometheus `/metrics` endpoint** (core, opt-in) — a plain-string exporter emitting Prometheus text exposition format 0.0.4 with **no third-party Prometheus client dependency**, served inside the dashboard's branched pipeline so it honors the path prefix. Exposes `hangfire_jobs_total` (counter), `hangfire_jobs_in_state_count{state}`, `hangfire_queue_length{queue}`, `hangfire_servers_count`, `hangfire_workers_count`, `hangfire_recurring_jobs_count` (gauges), and the `hangfire_job_duration_seconds` histogram (present only when an `IStorageMetricsProvider` is registered). Each metric family is computed independently — a family that fails is omitted while the rest are still emitted. Enabled via `options.Prometheus.Enabled` or `DashboardStorageOptionsBuilder.EnablePrometheusMetrics(...)`; defaults to `LocalOnly` authorization (`RequireDashboardAuth` or a dedicated `ScraperAuthorization` filter are opt-in). A sample Grafana dashboard ships at [`docs/grafana/hangfire-dashboard.json`](docs/grafana/hangfire-dashboard.json).
- **Read-only REST API** — a new NuGet package `a2n.Hangfire.Dashboard.RestApi` (not referenced by the core package) wrapping the existing providers with ASP.NET Core Minimal API endpoints mounted at `{prefix}/api/v1`: `GET /jobs` (search/filter + paging), `GET /jobs/{id}`, `GET /jobs/state/{state}`, `GET /queues`, and `GET /metrics/*` (404 "not available" when no metrics provider is registered, while query endpoints stay up). Responses use a shared `JobRecordDto` / `PagedResponse<T>` shape. Secured by JWT bearer authentication bound to the host's configured scheme (401 without a valid token, 403 when authenticated-but-unauthorized, no anonymous access by default), with an auto-generated OpenAPI document (Microsoft.AspNetCore.OpenApi on net9/net10, Swashbuckle on net8.0) that declares the bearer scheme. Invalid query parameters return HTTP 400 problem-details. Opt-in via `services.AddHangfireDashboardRestApi()` + `endpoints.MapHangfireDashboardRestApi(prefix)`.
- **CSV / JSON export** (core, opt-in) — a stream-based export of the current job search result set that respects the applied search criteria and does not grow memory with record count. CSV is RFC 4180-conformant; JSON reuses the REST API `JobRecordDto` shape. Served inside the branched pipeline at `{prefix}/export?format=csv|json` with a `Content-Disposition: attachment` header, gated by `Dashboard_Authorization` (401 before any record is streamed on failure), and available in read-only mode. Enabled via `options.Export.Enabled` or `DashboardStorageOptionsBuilder.EnableJobExport(...)`; `MaxRecords` caps the export size.

### Security

- Every new network-exposed endpoint (metrics, export, REST API) enforces an authorization check **before** returning any Hangfire data and is never anonymous by default; weakening an endpoint's authorization always requires an explicit configuration action. [docs/integrations-v2-6.md](docs/integrations-v2-6.md) documents each endpoint's default mode and how to change it.

### Internal

- New shared building blocks in the core package: a pure, dependency-free `W3CTraceParent` codec (linked into the OpenTelemetry package so both sides agree on the wire format), the shared `JobRecordDto` / `PagedResponse<T>` shapes and projection helpers, and the `Services/Prometheus` model + formatter + exporter.
- Test coverage: the 19 correctness properties from the design implemented as FsCheck property tests (≥100 iterations each; Prometheus exposition and CSV validated against independent parser oracles, export criteria fidelity against the provider result as model oracle), plus bunit component tests, endpoint/authorization unit tests, and `TestHost`/`WebApplicationFactory` integration tests across a new `a2n.Hangfire.Dashboard.OpenTelemetry.Tests` and `a2n.Hangfire.Dashboard.RestApi.Tests` project.
