...
- **Recurring Jobs grid columns are sortable ([#41](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/41)).** Click **Id**, **Job**, **Next Execution**, **Last Execution** or **Created** to sort ascending, and click again for descending. The active column shows a direction arrow and sets `aria-sort`. The sort applies to the complete filtered list before paging, so it spans every page, and it is kept when changing page and across the 10-second auto-refresh. Changing the sort returns to the first page and clears the selection, as paging does. Jobs without a value for the column (no next execution, never run, or a job type the dashboard can't resolve) are always listed last, whichever the direction, and ties are broken by id. Sorting by **Job** uses the displayed job name, so recurring jobs of the same type are grouped together. Until a header is clicked the grid keeps the storage order it had before. The new shared `SortableHeader` component can be reused by other tables.
  ...
  - The per-state job lists (Enqueued, Scheduled, Processing, Succeeded, Failed, Deleted, Fetched, Awaiting, Retries), Search and Tags are not sortable yet. Those pages receive one page at a time from Hangfire's monitoring API or the dashboard's storage queries, so a correct whole-list sort needs storage-side ordering rather than re-ordering the visible rows.
