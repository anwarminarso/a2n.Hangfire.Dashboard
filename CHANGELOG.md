...
- **Failed jobs record a failure fingerprint ([#53](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/53)).** This is the first step toward grouping failed jobs by exception. The new opt-in server filter `FailureFingerprintFilter`, registered with `config.UseDashboardFailureFingerprintFilter()`, stores a `FailureFingerprint` job parameter such as `v1:6eca44f1fc4dce28` on every job that enters the Failed state. The value is the first 16 hex characters of a SHA-256 over the exception type, the first line of the message with run-specific values (GUIDs, dates and times, hex values, e-mail addresses, IPv4 addresses, URL query strings, numbers) replaced by placeholders, and the top stack frame outside `System.`, `Microsoft.` and `Hangfire.`. The frame is kept without parameters, file or line number, with async, lambda and local-function names mapped to their containing method. Deadlocks with different process ids, or the same `NullReferenceException` rebuilt with its line moved, get one fingerprint. `Column 'Email' cannot be null` and `Column 'Phone' cannot be null`, or the same exception thrown from two different methods, get two. No UI uses it yet; grouping on the Failed page follows.
  ...
  - **The rules are public and versioned.** `FailureFingerprint.Compute` returns the fingerprint together with its parts (exception type, normalized message, top frame), and `FailureFingerprint.IsCurrentVersion` recognises values computed with the current rules. A rule change that alters fingerprints bumps the `v1:` prefix; a test pins an exact v1 value so such a change cannot slip through unnoticed.
