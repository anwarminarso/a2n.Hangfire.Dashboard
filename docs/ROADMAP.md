...
## v2.5.3 — Throttling visibility, DST-correct heatmap, storage portability ✅

**Goal**: Give operators visibility into Hangfire.Throttling primitives ([#30](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/30)), and fix two defects affecting existing installations. Kept on the v2.5.x line deliberately: the Throttling page is auto-detected and invisible to hosts that don't use the package, so it does not warrant renumbering v2.6 (Integrations) and v2.7 (Customization).

- ✅ **Throttling pages** ([#30](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/30), [#31](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/pull/31), contributed by [@d-lundy](https://github.com/d-lundy)) — `/throttling` listing semaphores (limit, description, live occupancy, holders), currently held mutexes, and fixed/sliding/dynamic rate-limit windows; `/throttling/semaphores/{id}` for per-holder detail. Orphaned holders are flagged in three cases (job record expired, final state reached without releasing, or Processing on a stale-heartbeat server) with a one-minute grace period on final states. Reads the storage format directly with no dependency on the Hangfire.Throttling assembly, following the Console/Tags precedent; the layout was established by runtime observation against SQL Server, Redis, and in-memory storage. Nav item auto-detects, with process-wide caching because the *negative* answer is the expensive one.
- ✅ **Heatmap DST correctness** — `BuildWindow` used `start.AddDays(7)`, which adds 168 hours of absolute time at the start's offset. Across a DST transition that drifts off local midnight: spring-forward ran an hour long, so in-window fires bucketed at day index 7 and were silently dropped by consumers that build cells for days 0–6; fall-back ended an hour early, leaving the last local hour of day 6 unprojected. The window is now seven local calendar days (167/168/169 absolute hours). Also resolved contradictory assertions in the test suite, where one property required a fixed 168-hour duration and another required an in-window day index of 0–6.
- ✅ **Storage portability** — `GetSetCount`, `GetRangeFromSet`, and `GetValueFromHash` are `virtual` on `JobStorageConnection` and throw `NotSupportedException` unless a storage overrides them. `TagsDataReader.GetTagsCount()` runs from `NavMenu`, so on such storages the throw failed the circuit and took down the whole shell rather than one counter. New `Storage/SetCounting` falls back to the `abstract` `GetAllItemsFromSet`; applied across `TagsDataReader`, `AuditLogService`, `HangfireMonitorService`, and `ConsoleDataReader`.
- ✅ **Continuous integration** — the repository had none (`publish.yml` only runs on `release: published`). A `Build & Test` workflow now builds and tests across net8.0/net9.0/net10.0 on every pull request, on pushes to `main`, and on demand, with PostgreSQL as a service container. The intermittently failing `StatCards` render-wait was stabilized first, since CI is only useful when green means something.

> **Detach deferred.** [#31](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/pull/31) also implemented Detach — the recovery action for orphaned holders — with audit entries. The read path rests on observed storage layout, but the keys Detach *writes* to (the semaphore holder set, the mutex registry pair, the mutex holder set) do not: the verification probe registers a semaphore without ever driving a job through it, and never acquires a mutex. A wrong read shows an empty column; a wrong write corrupts a concurrency invariant. Held until a probe run with `[Semaphore]`- and `[Mutex]`-attributed jobs confirms the holder-set layout and whether occupancy is tracked anywhere besides the holder set's cardinality. See the Stretch / Backlog entry.

---

## v2.5.4 — Recurring Jobs pagination, job detail links, collection-typed arguments ✅

**Goal**: Clear the three reported Recurring Jobs defects ([#37](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/37), [#38](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/38), [#39](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/39)) plus one editor bug surfaced while verifying them. All four are confined to the Recurring Jobs page and the parameter editor's type mapping, so they stay on the v2.5.x line.

- ✅ **Page-size selector now paginates** ([#37](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/37)) — `HandlePageSizeChanged` set `_pageSize`, but the table iterated all of `FilteredJobs` with no `Skip`/`Take` and the shared `Pager` was absent, so the selector changed nothing but itself. Both the active and Stopped tables now render a page with their own offset and `Pager`, matching the idiom on Retries / Failed / Succeeded. Two adjacent problems fixed at the same time: `ToggleAll` selected the entire filtered set, so "select all" + Delete could remove rows never displayed — it is now scoped to the visible page; and nothing re-clamped the offset when the list shrank (delete, trigger, stop/start, or the 10-second auto-refresh), which pagination newly made reachable as an empty trailing page under a stale "Showing X–Y of Z".
- ✅ **Recurring rows link to the latest execution** ([#38](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/38)) — the **Id** and **Last Execution** cells link to `jobs/details/{LastJobId}` when one exists, restoring the original dashboard's path from a recurring job to its latest run; never-run jobs stay plain text rather than rendering a dead link. Cells are linked rather than the whole `<tr>`, which also holds the selection checkbox and the Edit/Stop buttons.
- ✅ **Collection-typed arguments survive the editor** ([#39](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/issues/39)) — a `Dictionary<string, string>` argument was kept only while the JSON view was never left; Form view rendered it empty and saving persisted `null`. A dictionary is a concrete, visible, non-abstract class, so it passed `IsNestedObjectCandidate` and got the nested sub-form — which builds fields by reflecting over public writable properties, and a dictionary's entries are behind the indexer, not properties. Non-array `IEnumerable` implementations now map to `ParameterInputKind.Json`, which already round-trips through the serializer. Placed after the array branch (which always returns) and the `string` branch, so `T[]`, `byte[]`, and `string` are untouched.
- ✅ **Blank required parameters no longer block the view toggle** — surfaced while reproducing #39 and unrelated to collection types. `ValidateJsonForMethod` answered two different questions through one gate — is this JSON structurally usable, and is this form complete — and the JSON ▸ Form switch consumed that gate, so toggling views on any method with a required parameter was rejected with `Parameter 'x' is required.`. A `requireAllPresent` flag now separates them: the switch passes `false` and is blocked only by structural problems, while `EmitJsonState` keeps `true` so the state feeding `JobBuilder`'s submit gate still treats a blank required parameter as invalid. Switching a view is not submitting; the enforcement belongs at save.

> **Test coverage.** `ParameterInputMapperProperties` covered every row of the mapping table except collections, which is why #39 could ship unnoticed; dictionary, list, set, and `IEnumerable<T>` cases were added. `InvalidJsonBlocksFormProperties` drops `required-missing` from its invalid-switch candidates and gains a regression test asserting the switch succeeds while overall validity stays `false`. The #39 signature also lives in the sample app as `report-email-dictionary-args`, so the path is exercised by hand as well.
...
- [ ] **Throttling Detach** (deferred from v2.5.3, [#31](https://github.com/anwarminarso/a2n.Hangfire.Dashboard/pull/31)) — detaching an orphaned holder from a semaphore or mutex to free a leaked slot, audit-logged and hidden when read-only. Implementation exists and was reviewed; it is held only for want of storage-format evidence on the write path. **Unblocked by**: a Hangfire.Throttling probe run with `[Semaphore]`- and `[Mutex]`-attributed jobs that dumps `Set`, `Hash`, `Counter`, and `AggregatedCounter` for `sync:%` *while a slot is held* and again after release — confirming the holder-set keys and, critically, whether occupancy is tracked anywhere other than the holder set's cardinality. If a separate counter exists, a holder-set removal alone would free the slot in the UI while leaving it consumed in storage, making the leak permanent and invisible. Restoring the feature is a revert of the removal commit plus whatever the evidence corrects. This does **not** require the 5+ request rule — it is evidence-gated, not demand-gated.
